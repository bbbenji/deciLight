...
void setup()
{
  ...
  while (xQueueReceive(mic.sampleQueue(), &samples, portMAX_DELAY))
  {
    ...
    float sum_sqr_SPL = MIC_EQUALIZER.calculateSumOfSquares(samples, samples, SAMPLE_COUNT);
    // Sum of squares of equalized + weighted mic samples
    float sum_sqr_weighted = WEIGHTING.calculateSumOfSquares(samples, samples, SAMPLE_COUNT);
